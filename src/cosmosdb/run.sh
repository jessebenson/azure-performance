...
usage() {
    ...
    echo ""
    echo "Example:"
    echo "  $0 -a $ACR -g $RESOURCE_GROUP -l $LOCATION -w $WORKLOAD -t $THREADS -s $DURATION -c $CPU -m $MEMORY"
    echo ""
    exit
}
...
cosmosdb=`az cosmosdb list --resource-group $RESOURCE_GROUP | jq ".[0]"`
NAME=`echo $cosmosdb | jq .name | tr -dt '"'`
cosmosdb_endpoint=`echo $cosmosdb | jq '.documentEndpoint' | tr -dt '"'`
cosmosdb_database="database"
cosmosdb_collection="collection"
