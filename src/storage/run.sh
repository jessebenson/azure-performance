...
# Get Storage details
storage=`az storage account list --resource-group $RESOURCE_GROUP | jq ".[0]"`
NAME=`echo $storage | jq .name | tr -dt '"'`
storage_connectionstring=`az storage account show-connection-string --resource-group $RESOURCE_GROUP --name $NAME | jq .connectionString | tr -dt '"'`
...
dotnet publish $SRC/storage.csproj -c Release -o $SRC/bin/publish > /dev/null 2>&1
